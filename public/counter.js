...
function changeTheme() {
  ...
  submit.style.color = colorThemeFG[themeStyle];
  submit.style.backgroundColor = colorThemeBG[themeStyle];
  submit.style.borderColor = colorThemeFG[themeStyle];
  body.style.backgroundColor = bgColor[themeStyle];
}

themes.addEventListener('click', changeTheme);

function submitDuroodCount(csrfToken) {
  const duroodCount = document.getElementById('durood-count');
  fetch('http://localhost:3000/submit-count', {
    method: 'POST',
    body: JSON.stringify({
      count: +duroodCount.innerText,
    }),
    headers: {
    ...
    .then((result) => {
      alertMsg.getElementsByTagName('h2')[0].innerText = 'success';
    ...
    .catch((err) => {
      alertMsg.getElementsByTagName('h2')[0].innerText =
